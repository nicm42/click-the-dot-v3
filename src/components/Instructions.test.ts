...
test('Text is correct when no shape has been selected', () => {
  ...
  expect(screen.getByText('Click or press spacebar on the shape.')).toBeInTheDocument();
  expect(screen.getByText('Click or press spacebar again when it\'s twice the size.')).toBeInTheDocument();
})
...
test('Text is correct when shape is a circle', () => {
  ...
  expect(screen.getByText('Click or press spacebar on the circle.')).toBeInTheDocument();
  expect(screen.getByText('Click or press spacebar again when it\'s twice the size.')).toBeInTheDocument();
})
